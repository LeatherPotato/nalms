...
class CreationForm extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    fetch(GLOBALS.serverURL.concat("/create_book/"), CreateBookRequestOptions)
      ...
      .then(
        fetch(GLOBALS.serverURL.concat("/get_book/"), {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    return (
      <div>
        <form onSubmit={this.handleSubmit}>
          ...
          {this.state.genre == -1 ? (
            <span>
              <label htmlFor="genreName">newGenreName</label>
              <input
                type="text"
                name="genreName"
                onChange={this.handleChanges}
...
const genreOptions = (genres, option) => {
  // option parameter specifies whether i want the value to be genreName or genreId for the function its supposed to be added in (book creation adn book searching respectively)
  // option paramaeter also specifies whether the default option should be "ANY" or "NEW" instead of "ANY/NEW"
  console.log("GENRE OPTIONS FUNCT START");
  // console.log(genres)
  let returnGenreOptions = [<option value="-1">{option === 1 ? "NEW" : "ANY"}</option>];
  for (let index = 0; index < genres.length; index++) {
    let genre = genres[index];
    // console.log(genre)
    returnGenreOptions.push(
      <option value={option === 1 ? genre.GenreName : genre.GenreId}>
        {genre.GenreName}
      </option>
    );
  }
...
const Catalogue = () => {
  const [creationGenreOptions, setCreationGenreOptions] = useState(undefined);
  const [searchGenreOptions, setSearchGenreOtions] = useState(undefined);
  ...
  useEffect(async () => {
    fetch(GLOBALS.serverURL.concat("/get_genres/"), {
      ...
      .then((response) => response.json())
      .then((data) => {
        setCreationGenreOptions(genreOptions(data, 1));
        setSearchGenreOtions(genreOptions(data, 0));
      });
  }, []);
    ...
    <div id="page">
      {searchGenreOptions ? (
        <div id="catalogueSearchElement">
          ...
          <BookSearchElement genres={searchGenreOptions} />
        </div>
      ) : (
        <div id="page">
          <p>Loading...</p>
      ...
      {creationGenreOptions ? (
        <div id="catalogueCreateElement">
          <h1>Create Book</h1>
          <CreationForm genres={creationGenreOptions} />
        </div>
      ) : (
        <div id="page">
          <p>Loading...</p>
